...
### ¿La obesidad está asociada con el género?
obesidad<-datos$NObeyesdad
(genero<-datos$Gender)
(tabla<-(table(obesidad,genero)))
## test chisq cuadradi no están asociadas?
## p<0.05 si asociados si>0.05 no asociados
chisq.test(tabla)
### el genero,no está relacionado con la historia familiar de la obesidad?
familia<-datos$family_history_with_overweight
tabla2<-(table(familia,genero))
chisq.test(tabla2)
